...
CREATE TABLE IF NOT EXISTS supplier_payment_accounts (
  ...
  last_stripe_event_created_at TEXT CHECK (
    ...
    OR last_stripe_event_created_at ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}.*Z$'
  ),
...
CREATE TABLE IF NOT EXISTS wallet_funding_lots (
  ...
  original_amount BIGINT NOT NULL CHECK (original_amount BETWEEN 1 AND 100000000),
  available_amount BIGINT NOT NULL CHECK (
  ...
  funded_at TEXT NOT NULL CHECK (
    funded_at ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}.*Z$'
  ),
  updated_at TEXT NOT NULL CHECK (
    updated_at ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}.*Z$'
    AND updated_at >= funded_at
  ),
  CHECK (
...
CREATE OR REPLACE FUNCTION reject_wallet_funding_lot_identity_update()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  IF ROW(
    NEW.id, NEW.wallet_topup_id, NEW.organization_id,
    NEW.stripe_payment_intent_id, NEW.stripe_charge_id,
    NEW.original_amount, NEW.currency, NEW.funded_at
  ) IS DISTINCT FROM ROW(
    OLD.id, OLD.wallet_topup_id, OLD.organization_id,
    OLD.stripe_payment_intent_id, OLD.stripe_charge_id,
    OLD.original_amount, OLD.currency, OLD.funded_at
  ) THEN
    RAISE EXCEPTION 'wallet funding lot identity is immutable';
  END IF;
  RETURN NEW;
END;
$$;

CREATE OR REPLACE FUNCTION reject_wallet_funding_allocation_change()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'wallet funding allocations are immutable';
END;
$$;
...
DROP TRIGGER IF EXISTS wallet_funding_lots_guard_identity ON wallet_funding_lots;
CREATE TRIGGER wallet_funding_lots_guard_identity
  BEFORE UPDATE ON wallet_funding_lots
  FOR EACH ROW EXECUTE FUNCTION reject_wallet_funding_lot_identity_update();

DROP TRIGGER IF EXISTS wallet_funding_allocations_guard_update ON wallet_funding_allocations;
CREATE TRIGGER wallet_funding_allocations_guard_update
  BEFORE UPDATE ON wallet_funding_allocations
  FOR EACH ROW EXECUTE FUNCTION reject_wallet_funding_allocation_change();

DROP TRIGGER IF EXISTS wallet_funding_allocations_guard_delete ON wallet_funding_allocations;
CREATE TRIGGER wallet_funding_allocations_guard_delete
